...
export default function ApplicantDashboard() {
  ...
  const handleProfilePhotoUpload = async (file: File) => {
    try {
      ...
      const updatedProfile = await applicantAPI.uploadProfilePhoto(file)
      setProfile(updatedProfile)
    } catch (err: any) {
      console.error('Error uploading profile photo:', err)
      console.error('Error details:', err?.response?.data)
      const errorMessage = err?.response?.data?.detail || err?.message || 'Failed to upload profile photo. Please try again.'
  ...
  const renderContent = () => {
    ...
    switch (activeSection) {
      ...
      case 'profile':
        return (
          <ProfileSection profile={profile} onPhotoUpload={handleProfilePhotoUpload} />
        )
      case 'notifications':
        return <NotificationsCard applications={applications} />
      default:
        return null
    }
  }
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
          <div className="flex justify-between items-center h-14 sm:h-16">
            <div className="flex items-center gap-2 sm:gap-3">
              <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-primary-600">Training Portal</h1>
              <span className="text-xs sm:text-sm text-gray-500 hidden sm:inline">Applicant Dashboard</span>
            </div>
            {/* Mobile Menu Button - Right side of navbar */}
            <button
          ...
          <div className="flex-1 min-w-0">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                transition={{ 
                  duration: 0.35,
                  ease: [0.4, 0, 0.2, 1], // Custom cubic-bezier easing for smoother feel
                }}
              >
                {renderContent()}
              </motion.div>
